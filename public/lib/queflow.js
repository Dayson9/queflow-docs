...
function filterNullElements(input) {
  return input.filter((d) => selectElement(d.qfid));
}
...
function initiateNuggets(markup) {
  const nuggetRegex = /<([A-Z]\w*)\s*\{([\s\S]*?)\}\s*\/>/g,
    extNuggetRegex = /<([A-Z]\w*)\s*\(\{([\s\S]*?)\}\)\s*>([\s\S]*?)<\/([A-Z]\w*)>/g;

  let matches = extNuggetRegex.exec(markup);

  while (matches) {
    const [match, name, data, content] = matches;
    try {
