...
\` } />
  ...
  <P { txt: "When working with interpolated attributes, it's recommended to use quotes around them:" } />
  <CodeView { code: \`
...
\` } />
  <P { txt: "The [color] attribute uses a dynamic value, while the [font-family] attribute uses a static value. Whenever the value of [color] changes, the color of the element changes accordingly." } />

  <Heading { txt: "Event Syntax" } />
  <P { txt: "QueFlow uses a unique approach to writing event listeners, using double braces." } />
  <CodeView { code: \`
